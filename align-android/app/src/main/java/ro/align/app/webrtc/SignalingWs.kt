...
/** Protocol compatibil cu `server/call-signaling-server.mjs` (același ca `useWebRtcCall`). */
class SignalingWs(
    ...
    private fun dispatchMessage(text: String) {
        ...
        when (t) {
            "joined" -> {
                ...
                val peers = List(peersArr.length()) { peersArr.getString(it) }
                l.onJoined(rid, peers)
            }
            "session" -> l.onSession(j.getString("remoteUserId"), j.getBoolean("shouldOffer"))
            "offer" -> l.onOffer(j.getString("sdp"), j.optString("from", ""))
            "answer" -> l.onAnswer(j.getString("sdp"), j.optString("from", ""))
            "ice" -> {
            ...
            "error" -> l.onError(j.optString("code", null))
        }
    }
